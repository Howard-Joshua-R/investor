...
class AlphaVantage(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        smooting_window_size = 2500
        for di in range(0, 10000, smooting_window_size):
            scaler.fit(train_data[di:di+smooting_window_size,:])
            train_data[di:di+smooting_window_size,:] = scaler.transform(train_data[di:di+smooting_window_size,:])

        print('You normalize the last bit of remaining data')
        scaler.fit(train_data[di+smooting_window_size,:])
        train_data[di+smooting_window_size:,:] = scaler.transform(train_data[di+smooting_window_size:,:])
